...
public class SharkFollowOwnerGoal<T extends HostileEntity & TameableHostileEntity> extends Goal {
    private final SharkEntity tameable;
    ...
    public SharkFollowOwnerGoal(SharkEntity tameable, double speed, float minDistance, float maxDistance, boolean leavesAllowed) {
    ...
    public void tick() {
        ...
        if (--this.updateCountdownTicks <= 0) {
            ...
            if (this.tameable.squaredDistanceTo(this.owner) >= 500D) {
                ...
                double angle = Math.toRadians(this.tameable.getNum() * 120);

                double offsetX = 5f * Math.cos(angle);
                double offsetZ = 5f * Math.sin(angle);

                this.tameable.getMoveControl().moveTo(this.owner.getX() + offsetX, this.owner.getY() + 1, this.owner.getZ() + offsetZ, this.speed);
            }
    ...
    private boolean tryTeleportTo(int x, int y, int z) {
        if (Math.abs((double) x - this.owner.getX()) < 2.0D && Math.abs((double) z - this.owner.getZ()) < 2.0D) {
            ...
            this.tameable.refreshPositionAndAngles((double) x + 0.5D, (double) y, (double) z + 0.5D, this.tameable.bodyYaw, this.tameable.getPitch());
            this.navigation.stop();
            return true;
        }
    }
