...
public class ColorHelper {
   public static Color getColor(int decimal) {
      int red = Argb.getRed(decimal);
      int green = Argb.getGreen(decimal);
      int blue = Argb.getBlue(decimal);
   ...
   public static int getColor(Color color) {
      return Argb.getArgb(color.getAlpha(), color.getRed(), color.getGreen(), color.getBlue());
   }
   ...
   public static int getColor(float r, float g, float b, float a) {
      return Argb.getArgb((int)(a * 255.0F), (int)(r * 255.0F), (int)(g * 255.0F), (int)(b * 255.0F));
   }

   public static Color colorLerp(Easing easing, float pct, Color startColor, Color endColor) {
      pct = MathHelper.clamp(pct, 0.0F, 1.0F);
      ...
      int red = (int)MathHelper.lerp(ease, (float)br, (float)dr);
      int green = (int)MathHelper.lerp(ease, (float)bg, (float)dg);
      int blue = (int)MathHelper.lerp(ease, (float)bb, (float)db);
   ...
   public static Color multicolorLerp(Easing easing, float pct, List<Color> colors) {
      ...
      int index = (int)MathHelper.clamp(colorIndex, 0.0F, (float)colorCount);
   ...
   public static Color darker(Color color, int power, float factor) {
      ...
      return new Color(
         Math.max((int)((float)color.getRed() * FACTOR), 0),
         Math.max((int)((float)color.getGreen() * FACTOR), 0),
         Math.max((int)((float)color.getBlue() * FACTOR), 0),
         color.getAlpha()
      );
   }
