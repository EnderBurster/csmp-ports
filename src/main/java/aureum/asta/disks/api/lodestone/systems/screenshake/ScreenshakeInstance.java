   ...
   public float updateIntensity(Camera camera, Random random) {
      ...
      if (this.intensity2 != this.intensity3) {
         return percentage >= 0.5F
            ? MathHelper.lerp(this.intensityCurveEndEasing.ease(percentage - 0.5F, 0.0F, 1.0F, 0.5F), this.intensity2, this.intensity1)
            : MathHelper.lerp(this.intensityCurveStartEasing.ease(percentage, 0.0F, 1.0F, 0.5F), this.intensity1, this.intensity2);
      } else {
