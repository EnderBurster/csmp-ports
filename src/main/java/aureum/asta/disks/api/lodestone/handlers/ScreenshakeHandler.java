...
public class ScreenshakeHandler {
   ...
   public static void clientTick(Camera camera, Random random) {
      double sum = Math.min(INSTANCES.stream().mapToDouble(i1 -> (double)i1.updateIntensity(camera, random)).sum(), ClientConfig.SCREENSHAKE_INTENSITY);
   ...
   public static float randomizeOffset(int offset) {
      ...
      float sampled = (float)sampler.sample(
            (double)(((float)(MinecraftClient.getInstance().world.getTime() % 24000L) + MinecraftClient.getInstance().getTickDelta()) / intensity),
            (double)offset,
            0.0
         )
         * 1.5F;
