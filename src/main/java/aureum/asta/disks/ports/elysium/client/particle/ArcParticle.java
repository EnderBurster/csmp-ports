...
public class ArcParticle extends Particle {
   ...
   protected ArcParticle(
      ClientWorld clientLevel,
      BufferBuilderStorage renderBuffers,
      double x,
      double y,
      double z,
      double targetX,
      ...
      double targetZ,
      float scale,
      @Nullable Entity entityToFollow
   ) {
   ...
   public void buildGeometry(VertexConsumer vertexConsumer, Camera camera, float tickDelta) {
      Immediate bufferSource = this.renderBuffers.getEntityVertexConsumers();
      VertexConsumer buffer = bufferSource.getBuffer(RenderLayer.getLightning());
      MatrixStack poseStack = new MatrixStack();
      poseStack.push();
      poseStack.translate(this.x - camera.getPos().x, this.y - camera.getPos().y, this.z - camera.getPos().z);
      if (this.entityToFollow != null) {
         Vec3d offset = this.entityToFollow.getLerpedPos(tickDelta).subtract(this.offsetFromEntity);
         poseStack.translate(offset.getX(), offset.getY(), offset.getZ());
      }

      poseStack.scale(this.scale, this.scale, this.scale);
      Vec3d targetVec = new Vec3d(this.targetX - this.x, this.targetY - this.y, this.targetZ - this.z).multiply(1.0 / (double)this.scale);
      if (this.age % 4 != 0) {
         Random random = Random.create(this.seed + (long)(this.age / 9));
         float[][] nodePositions = generateNodePositions(random, targetVec);
         drawLightning(poseStack, buffer, nodePositions, random, 0, 0, this.getMinTreeDepth(tickDelta), this.getMaxTreeDepth(tickDelta));
      }
         ...
         for (int i = 0; i < nodePositions.length - 1; i++) {
            ...
            if (i > 0 && i < nodePositions.length - 2 && random.nextFloat() < 0.4F) {
               Vec3d newTarget = new Vec3d(
                     (double)pos[0] * 0.5 + (double)random.nextFloat(),
                     (double)pos[1] * 0.5 + (double)random.nextFloat(),
                     (double)pos[2] * 0.5 + (double)random.nextFloat()
                  )
                  .multiply(5.0);
               float[][] newNodePositions = generateNodePositions(random, newTarget);
               poseStack.push();
               poseStack.translate((double)pos[0], (double)pos[1], (double)pos[2]);
   ...
   private static float[][] generateNodePositions(Random random, Vec3d target) {
      Vec3d perpendicular1 = target.crossProduct(new Vec3d(1.0, 0.0, 0.0)).normalize();
      ...
      for (int i = 0; i < nodePositions.length; i++) {
         ...
         if (i == 0) {
            ...
            Vec3d offset = perpendicular1.multiply((double)random.nextFloat())
               .add(perpendicular2.multiply((double)random.nextFloat()))
               .add(target.multiply((double)(progress + random.nextFloat() * maxMovement)));
   ...
   private static void drawCuboid(
      Matrix4f matrix4f, VertexConsumer vertexConsumer, float xStart, float yStart, float zStart, float xEnd, float yEnd, float zEnd, float radius, float alpha
   ) {
      drawQuad(
         vertexConsumer,
         matrix4f,
   ...
   private static void drawSingleSidedQuad(
      VertexConsumer vertexConsumer,
      Matrix4f matrix4f,
      float x1,
      float x2,
   ...
   public static class Provider implements ParticleFactory<ArcParticleOption> {
      public Particle createParticle(ArcParticleOption type, ClientWorld level, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
         ArcParticle particle = new ArcParticle(
            level,
            ...
            type.targetZ(),
            type.scale(),
            level.getEntityById(type.entityId())
         );
