...
public class ElysiumPrismRenderer implements BlockEntityRenderer<ElysiumPrismBlockEntity> {
   ...
   public ElysiumPrismRenderer(Context context) {
   }

   public void render(
      ElysiumPrismBlockEntity blockEntity, float partialTick, MatrixStack poseStack, VertexConsumerProvider bufferSource, int packedLight, int packedOverlay
   ) {
      poseStack.push();
      int value = (Integer)blockEntity.getCachedState().get(ElysiumMachines.ELYSIUM_POWER);
      ...
      if (laserEnd != null && value > 0) {
         ...
         poseStack.translate(-0.5, -0.5, -0.5);
         if (value == 4) {
            poseStack.translate(RANDOM.nextGaussian() * 0.01, 0.0, RANDOM.nextGaussian() * 0.01);
         }
         ...
         renderLaser(
            ...
            time,
            beamSizeWobble,
            ALPHA_BY_POWER[value],
            Math.min(
               rayStart.distanceTo(MinecraftClient.getInstance().cameraEntity.getEyePos()),
               rayEnd.distanceTo(MinecraftClient.getInstance().cameraEntity.getEyePos())
            )
         );
      }
      poseStack.pop();
   ...
   public boolean shouldRenderOffScreen(ElysiumPrismBlockEntity blockEntity) {
      return true;
   }
   ...
   public static void renderBeaconBeam(
      ...
      float k = colors[1];
      float l = colors[2];
      poseStack.multiply(Vec3f.POSITIVE_Y.getDegreesQuaternionf(f * 2.25F - 45.0F));
      float m = 0.0F;
      float p = 0.0F;
      float q = -beamRadius;
      float r = 0.0F;
      ...
      renderPart(
         poseStack,
         bufferSource.getBuffer(GlowEffectManager.getRenderType(RenderLayer.getBeaconBeam(beamLocation, false))),
         j,
         k,
         l,
         outsideAlpha,
      ...
      renderPart(
         poseStack,
         bufferSource.getBuffer(RenderLayer.getBeaconBeam(beamLocation, true)),
         j,
         k,
         l,
         alpha,
   ...
   private static void renderSideQuad(
      Matrix4f pose,
      Matrix3f normal,
      VertexConsumer consumer,
      float red,
   ...
   private static void renderCapQuad(
      Matrix4f pose,
      Matrix3f normal,
      VertexConsumer consumer,
      float red,
