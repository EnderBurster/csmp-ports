...
public class LongswordRelevationComponent implements AutoSyncedComponent, CommonTickingComponent, AmariteLongswordItem.LongswordMode {
   ...
   private final List<Entity> relevationTargets = new ArrayList<>();
   private int relevationCharge = 8000;
   public boolean relevationActive = false;
   public int relevationActiveTicks = 0;

   public LongswordRelevationComponent(PlayerEntity player) {
      this.player = player;
   }
   ...
   public void serverTick() {
      if (this.relevationActive && !this.relevationTargets.isEmpty()) {
         ...
         for (int i = 0; i < this.relevationTargets.size(); i++) {
            Entity target = this.relevationTargets.get(i);
   ...
   public void tick() {
      if (!this.relevationActive) {
         ...
         if (this.relevationCharge <= 0
            || this.relevationActiveTicks > 2 && this.player.mialib$startedAttacking()
   ...
   public void absorbDamage(float base) {
      this.relevationCharge += (int)(base * 20.0F);
   ...
   public void useAbility() {
      if (this.relevationCharge >= 54 || this.player.isCreative()) {
   ...
   public float getChargeProgress() {
      return (float)this.relevationCharge / 1444.0F;
   }

   @Override
   public String getTranslationKey() {
      return AmariteItems.AMARITE_LONGSWORD.getTranslationKey() + ".relevation";
   }

   public void readFromNbt(@NotNull NbtCompound tag) {
      this.relevationCharge = tag.getInt("relevationCharge");
      this.relevationActive = tag.getBoolean("relevationActive");
   }

   public void writeToNbt(@NotNull NbtCompound tag) {
      if (this.relevationCharge != 0) {
         tag.putInt("relevationCharge", this.relevationCharge);
      }

      if (this.relevationActive) {
         tag.putBoolean("relevationActive", true);
      }
   }
}
