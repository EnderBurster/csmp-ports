...
public class MRegistry {
   ...
   public Item registerItem(String path, Item item, @NotNull ItemGroup... groups) {
      return this.register(path, item);
   }

   public Item registerVanillaItem(String path, Item item, @NotNull ItemGroup... groups) {
      return this.register(new Identifier("minecraft", path), item);
   }

   @SafeVarargs
   public final Item registerItem(String path, Item item, @NotNull Consumer<Item>... groups) {
      for (Consumer<Item> group : groups) {
   ...
   @SafeVarargs
   public final Block registerBlockWithItem(String path, Block block, @NotNull Consumer<Item>... groups) {
      this.register(path, block);
      this.registerItem(path, new BlockItem(block, new FabricItemSettings()), groups);
      return block;
   }

   public <T extends MobEntity> EntityType<T> registerEntity(String path, EntityType<T> entity, @Nullable MRegistry.EggData eggData) {
      return this.registerEntity(path, entity, null, eggData);
   }

   public <T extends LivingEntity> EntityType<T> registerEntity(String path, EntityType<T> entity, @Nullable Builder attributes) {
   ...
   public <T extends MobEntity> EntityType<T> registerEntity(
      String path, EntityType<T> entity, @Nullable Builder attributes, @Nullable MRegistry.EggData eggData
   ) {
      this.register(path, entity);
      if (attributes != null) {
         this.entityAttributeRegistrations.add(() -> FabricDefaultAttributeRegistry.register(entity, attributes));
      ...
      if (eggData != null) {
         SpawnEggItem egg = new SpawnEggItem(entity, eggData.primaryColor, eggData.secondaryColor, new FabricItemSettings());
   ...
   public <T> T register(Identifier id, T thing) {
      if (this.built) {
         throw new IllegalStateException("Tried to register " + id + " to the " + this.namespace + " MRegistry after it was built!");
      } else {
         boolean registered = false;

         for (Entry<Class<?>, Registry<?>> registry : this.registries.entrySet()) {
            if (registry.getKey().isInstance(thing)) {
               this.objects.putIfAbsent(registry.getValue(), new HashMap<>());
               Map<Identifier, Object> map = this.objects.get(registry.getValue());
               if (map.containsKey(id)) {
                  throw new IllegalStateException(
                     "Failed to register " + id + " as it already exists in the " + registry.getValue() + " " + this.namespace + " MRegistry."
                  );
               }
         ...
         if (!registered) {
            throw new IllegalStateException("Failed to register " + id + " to the " + this.namespace + " MRegistry!");
         } else {
            return thing;
         }
      }
   ...
   public static record EggData(int primaryColor, int secondaryColor) {
