...
import aureum.asta.disks.damage.HellforkHeatDamageSource;
import aureum.asta.disks.entity.ImpaledTridentEntity;
    ...
    public void onStoppedUsing(ItemStack stack, World world, LivingEntity user, int remainingUseTicks) {
        ...
        if (user instanceof PlayerEntity player && user.isUsingRiptide() && stack.getItem() == ImpaledItems.SOULFORK) {
            if (player.experienceLevel <= 0) {
                user.damage(HellforkHeatDamageSource.of(world, HellforkHeatDamageSource.HELLFORK_HEAT), 2f);
