...
import aureum.asta.disks.ports.mason.init.MasonComponents;
import aureum.asta.disks.ports.mason.init.MasonObjects;
import aureum.asta.disks.mixin.ports.mason.EntityAccessor;
...
public class BoneflyEntity extends HostileEntity implements GeoAnimatable, TameableHostileEntity {
    ...
    public double getAltitude(int limit) {
        ...
        for (int i = 0; i <= limit && mutable.getY() > 0 && !this.getWorld().getBlockState(mutable.move(Direction.DOWN)).getMaterial().blocksMovement(); i++);
