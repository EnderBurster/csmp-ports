package aureum.asta.disks.ports.mason.entity;

import aureum.asta.disks.ports.mason.entity.goal.SoulmouldAttackLogicGoal;
import aureum.asta.disks.ports.mason.entity.goal.SoulmouldDashSlashGoal;
import aureum.asta.disks.ports.mason.entity.goal.TamedAttackWithOwnerGoal;
import aureum.asta.disks.ports.mason.entity.goal.TamedTrackAttackerGoal;
...
public class SoulmouldEntity extends HostileEntity implements TameableHostileEntity, GeoAnimatable {
    ...
    private void cycleActionState(PlayerEntity player) {
        if(getActionState() == 1) {
            setActionState(2);
            player.sendMessage(Text.translatable("Soulmould", getWorld().getRegistryKey().getValue().getPath()).setStyle(Style.EMPTY.withColor(Formatting.DARK_RED).withObfuscated(true).withFont(new Identifier("minecraft", "default"))), true);
        } else if(getActionState() == 0) {
            setActionState(1);
            player.sendMessage(Text.translatable("info.aureum-asta-disks.mould_activate", getWorld().getRegistryKey().getValue().getPath()).setStyle(Style.EMPTY.withColor(Formatting.AQUA)), true);
        } else if(getActionState() == 2) {
            setActionState(0);
            player.sendMessage(Text.translatable("info.aureum-asta-disks.mould_deactivate", getWorld().getRegistryKey().getValue().getPath()).setStyle(Style.EMPTY.withColor(Formatting.DARK_GRAY)), true);
        }
    }
    ...
    protected void playStepSound(BlockPos pos, BlockState state) {
        if(getWorld() instanceof ServerWorld server)
            server.getServer().getPlayerManager().sendToAround(null, this.getX(), this.getY(), this.getZ(), 32.0, server.getRegistryKey(), new PlaySoundFromEntityS2CPacket(RegistryEntry.of(SoundEvents.ENTITY_IRON_GOLEM_STEP), this.getSoundCategory(), this, 32.0F, 1.0F, 69L));
    }
