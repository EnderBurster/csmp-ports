...
import aureum.asta.disks.ports.blast.common.entity.ShrapnelBlockEntity;
...
public class BonesburrierExplosion extends CustomExplosion {
        ...
        if (bl) {
            ...
            for (BlockPos blockPos : this.affectedBlocks) {
                ...
                for (DiamondOfProtection dia : ((CharterWorldComponent)this.world.getComponent(CharterComponents.CHARTER)).diamonds) {
                    blC2 = dia.isPosInside(blockPos) && (!(this.getCausingEntity() instanceof PlayerEntity pl) || !dia.isOwner(pl));
                }

                if (canPlace(blockPos) && canExplode(blockPos) && !blC2) {
                    BlockState blockState = this.world.getBlockState(blockPos);
            ...
            for (Pair pair : objectArrayList) {
                if(shouldDrop(this.world, (BlockPos) pair.getSecond(), (ItemStack) pair.getFirst()))
                {
    ...
    public static void tryMergeStack(ObjectArrayList<Pair<ItemStack, BlockPos>> stacks, ItemStack stack, BlockPos pos) {
        int i = stacks.size();
        for (int j = 0; j < i; ++j) {
