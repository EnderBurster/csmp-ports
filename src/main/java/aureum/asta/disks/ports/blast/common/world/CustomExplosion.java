package aureum.asta.disks.ports.blast.common.world;
...
public class CustomExplosion extends Explosion {
    ...
    public void affectWorld(boolean boolean_1) {
        ...
        if (boolean_2) {
            ...
            while (affectedBlocks.hasNext()) {
                ...
                if (canExplode(blockPos)) {
                    ...
                    if (!blockState.isAir() && blockState.getFluidState().isEmpty() || blockState.getBlock() instanceof FluidFillable) {
                        ...
                        for (DiamondOfProtection dia : ((CharterWorldComponent)this.world.getComponent(CharterComponents.CHARTER)).diamonds) {
                            boolean bl = dia.isPosInside(blockPos) && (!(this.getCausingEntity() instanceof PlayerEntity pl) || !dia.isOwner(pl));
                            if (bl && !(this.world.getBlockState(blockPos).getBlock() instanceof CharterStoneBlock)) {
                                ((CharterWorldComponent)this.world.getComponent(CharterComponents.CHARTER))
                                        .addBlockChange(
                                                new QueuedBlockChange(!blockState.isIn(CharterBlocks.BREAKABLES) && !(blockState.getBlock() instanceof PlantBlock) ? 200 : 204, blockPos, blockState)
                                        );
                                break;
                            }
                        }
                        ...
                        for (WaterBarrier bar : (this.world.getComponent(AureumAstaDisks.KYRATOS)).barriers) {
                            ...
                            if (bl && !(this.world.getBlockState(blockPos).getBlock() instanceof BarrierBlock)) {
                                (this.world.getComponent(AureumAstaDisks.KYRATOS)).addBlockChange(new QueuedBlockChange(!blockState.isIn(CharterBlocks.BREAKABLES) && !(blockState.getBlock() instanceof PlantBlock) ? 200 : 204, blockPos, blockState));
                                break;
                            }
                        }
            ...
            for (Pair<ItemStack, BlockPos> itemStackBlockPosPair : objectArrayList) {
                ...
                if(shouldDrop(this.world, pair.getSecond(), pair.getFirst()))
                {
    ...
    protected boolean shouldDrop(World world, BlockPos pos, ItemStack stack)
    {
        boolean bl = false;

        for (DiamondOfProtection dia : ((CharterWorldComponent)world.getComponent(CharterComponents.CHARTER)).diamonds) {
            bl = dia.isPosInside(pos)
                    && (!(this.getCausingEntity() instanceof PlayerEntity pl) || !dia.isOwner(pl))
