...
public class ChainsEntity extends Entity implements LockedTransport {
   @Nullable
   public UUID ownerUuid;
   public final boolean temp;

   public ChainsEntity(EntityType<? extends ChainsEntity> type, World world) {
      super(type, world);
   ...
   public ChainsEntity(World world) {
      super(CharterEntities.CHAINS, world);
      this.intersectionChecked = true;
      this.temp = false;
      this.ownerUuid = null;
   }

   public ChainsEntity(World world, @Nullable UUID uuid) {
      super(CharterEntities.CHAINS, world);
      this.intersectionChecked = true;
      this.temp = false;
      this.ownerUuid = uuid;
   ...
   public ChainsEntity(World world, @Nullable UUID uuid, boolean temporary) {
      super(CharterEntities.CHAINS, world);
      this.intersectionChecked = true;
      this.temp = temporary;
      this.ownerUuid = uuid;
   ...
   protected Vec3d positionInPortal(Axis portalAxis, Rectangle portalRect) {
   ...
   public void tick() {
      super.tick();
      if (!this.hasPassengers() || this.temp && this.age >= 3600) {
